...
| 维度 | 说明 |
|------|------|
| 目标读者 | Rust 高级开发、区块链架构、基础设施 / 安全 / 数据平台角色 |
| 案例来源 | 公链、跨链桥、DeFi 协议、预言机、钱包等基础设施级项目 |
| 时间窗口 | 2016–2023 典型事件，量化指标来自公开渠道 |
| 用途 | 面试案例分析、系统设计讨论、经验反思 |

```mermaid
timeline
    title 区块链基础设施案例关键时间线
    2016 : DAO攻击推动重入防护
    2018 : Chainlink 预言机网络扩张
    2020 : Ethereum 2.0 信标链上线
    2021 : Solana 中断、Poly Network 黑客事件
    2022 : Terra/Luna 崩溃、Ronin Bridge 被攻破
    2023 : dYdX v4 独立链挑战、MakerDAO Endgame 争议
```
...
| 成功标准 | 指标 | 通过阈值 |
|-----------|------|-----------|
| 覆盖度 | 面试中涉及的类别数量 | ≥3 个类别，至少 1 个失败案例 |
| 分析深度 | 候选人完成结构化分析所需时间 | 10–15 分钟内完成完整框架 |
| 证据引用 | 量化指标与参考文献数量 | ≥2 个指标 + ≥1 条 G/S/T/L/A 引用 |
