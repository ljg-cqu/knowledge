...
| 维度             | 内容                                                                 |
|------------------|----------------------------------------------------------------------|
| 问题数量与范围   | 10 组 Q&A，覆盖 Rust、区块链原理、智能合约、公链、数据结构、加密、安全、系统设计等 |
| 难度分布         | 约 10% 基础 / 40% 中级 / 50% 高级，整体偏中高级                       |
| 结构组成         | 每题包含场景、答案要点、评分标准、参考资料                           |
| 评估辅助         | 附表汇总难度、类型、评分权重和核心考察点，便于快速筛选与比较         |
...
```math
总分 = \sum_{i=1}^{10} \left( \frac{\text{题目得分}_i}{5} \times \text{权重}_i \right)
```

**示例评分伪代码**

```python
weights = [0.10, 0.10, 0.15, 0.15, 0.10, 0.10, 0.15, 0.05, 0.05, 0.05]
scores = [...]  # 每题 0-5 分

total = 0.0
for s, w in zip(scores, weights):
    total += (s / 5.0) * (w * 100)  # 计算加权得分（0-100）
```

```mermaid
flowchart TD
    S[单题评分 0-5 分] --> N[换算为百分比]
    N --> W[乘以题目权重]
    W --> SUM[求和得到总分 0-100]
    SUM --> DECISION{是否大于等于 80 分?}
    DECISION -->|是| H[推荐录用]
    DECISION -->|否且大于等于 60| M[视情况补充面试或培养]
    DECISION -->|小于 60| L[不推荐录用]
```
