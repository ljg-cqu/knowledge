...
### 新闻背景
2024年3月，Ethereum 完成 Dencun 升级，引入 EIP-4844（Proto-Danksharding），显著改变数据可用性层（Data Availability Layer），降低 Layer2 rollup 的数据发布成本。以部分 rollup 公布的数据为例，数据成本下降约 80%–90%，为 DEX 等应用的整体 gas 账单带来可观下降。该升级通过引入“blob”数据结构，优化数据存储和传输方式，从而减少 gas 消耗。在使用 Rust 编写的以太坊客户端（如 reth）中，实现细节通常围绕高效内存管理和并发处理展开，以支撑更高的 TPS 和更低的延迟。
...
④ **度量与权衡**：
   - 成功标准可以设为：在 EIP-4844 生效后，将典型 swap 交易的平均 gas 成本在原有基础上再降低 ≥30%，同时保持 p95 交易确认延迟不劣于升级前。
...
④ **度量与成功标准**：
   ...
   - 监控主网或测试网中“区块重放失败次数”“共识停顿时长”等指标，设定明确阈值（如任何节点连续若干个 slot 无法投票即触发熔断 / 降级），保证即便出现 bug 也不会演变为全网宕机。
...
### 新闻背景
2024年，Jump Crypto 团队公布了 Firedancer 验证器的性能测试结果：在专用硬件环境下可达到约 30 万 TPS 的处理能力（具体数字以官方报告为准），相比现有 Solana 验证器有明显提升。Firedancer 主要使用 C 语言和高度优化的事件驱动 I/O、内存布局来减少系统开销；如果在 Rust 中实现类似高性能验证器，则需要在内存分配、并发调度和网络 I/O 等方面做针对性优化，而不能仅依赖默认配置。
...
④ **性能指标与风险控制**：
   - 针对不同硬件档位设定清晰目标，例如在通用云服务器上达到稳定数万 TPS、p99 区块传播延迟 < 500ms，同时 CPU 利用率和内存占用保持在可运维范围内。
...
④ **风险与成功标准**：
   - 为钩子执行设定明确的时间和资源上限，例如单次钩子执行时间不超过若干毫秒、内存分配量不超过约定阈值，并在运行时对超限行为进行熔断或回滚。
