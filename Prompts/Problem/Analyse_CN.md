...
在进入第 1 部分之前，先输出一个简短的小节 **“上下文回顾”**，使答案对单个问题实例自包含：
- 用一行写出本次具体问题实例的标题，例如：`问题 1：面向高频 / 实时场景的 MPC 签名时延问题`；
- 用 2–5 个要点重述关键上下文：当前现状、主要痛点、目标 / 关键指标、硬性约束，以及【输入】中最重要的背景事实；
- 然后再按下面的第 1–10 部分依次展开分析。
