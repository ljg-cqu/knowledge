...
Framework for generating interview question banks that assess business-to-architecture translation capabilities, optimized for LLM-friendly prompt design.

Related: See [Guidelines for LLM-Friendly Prompts](Guidelines_for_LLM-Friendly_Prompts.md) for detailed principles applied in this prompt.

## Guideline Checklist Reference

This checklist summarizes the LLM-Friendly Guidelines that this prompt follows:

1. **Define Context**: State scope, constraints, assumptions clearly in the prompt.
2. **Ensure Clarity**: Use clear explanations and avoid undefined jargon.
3. **Maintain Precision**: Use specific, consistent terminology throughout.
4. **Stay Relevant**: Focus only on topic information, exclude tangential content.
5. **Use MECE Structure**: Ensure complete, non-overlapping coverage of topics.
6. **Provide Sufficient Coverage**: Include all necessary aspects of the topic.
7. **Include Multiple Perspectives**: Present different viewpoints where appropriate.
8. **Ensure Depth**: Cover topics with sufficient detail and thoroughness.
9. **Ensure Accuracy**: Verify factual correctness and cross-check information.
10. **Prioritize Credible Sources**: Use authoritative, high-quality, tested sources.
11. **Focus on Significance**: Prioritize important information over trivial details.
12. **Maintain Logic**: Demand coherent reasoning and sound structure.
13. **Ensure Fairness**: Request balanced perspectives, acknowledge all assumptions and biases.
14. **Use Concision**: Remove redundancy and include only essential information.
15. **Provide Structure**: Use lists, tables, diagrams, formulas, and code blocks.
16. **Use Structured Format**: Request output with TOC linking to sections.
17. **Include Evidence**: Cite credible sources and flag uncertainties.
18. **Request Validation**: Require reasoning, self-review, and error checks.
19. **Ensure Practicality**: Make guidance actionable and implementable.
20. **Define Success Criteria**: Specify measurable, concrete outcomes.

---

## Purpose and Scope

**Purpose**: Generate high-quality interview question banks that assess business-to-architecture translation capabilities.

**Scope**: 25–30 Q&A pairs targeting senior/architect/expert technical leaders.

**Audience**: Technical leaders evaluating candidates' ability to translate business requirements into architectural decisions.

**Use Cases**: Interview preparation, assessment of business-technical translation skills, architecture discussion frameworks.
...
- Evaluating candidates' business-to-architecture translation abilities
- Creating scenario-based interview questions
- Assessing strategic thinking and constraint analysis skills
- Building question banks for senior/architect/expert roles

**Do not use when**:
- Assessing pure technical skills (algorithms, coding, specific frameworks)
...
Provide or accept the following:

| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| question_type | enum | all | [strategic_modeling, value_risk_analysis, documentation_visualization, organizational_dynamics, architectural_translation, evolution_adaptation] |
| business_context | string | required | Industry, business model, constraints |
| data_sources | array | [] | Reference materials available |
| constraints | object | {} | Time limits, word count, format requirements |
| audience_level | enum | architect | [senior, architect, expert] |
| output_format | enum | text | [text, json, structured_markdown] |
| confidence_policy | enum | medium | [high, medium, low] - uncertainty threshold |
| ask_policy | enum | ask_when_needed | [always_sufficient, ask_when_needed, proceed_with_assumptions] |

---

## Output Requirements and Format

**Text Format**: Questions with 150-300 word answers, cited sources, visual artifacts.
...
```json
{
  "questions": [
    {
      ...
      "type": "string",
      "key_insight": "string",
      "answer": "string (150-300 words)",
      "supporting_artifacts": {
        "diagram": "Mermaid code or description",
        "table": "markdown table",
        "formula": "mathematical formula with definitions"
      },
      "sources": ["[Ref: ID]"],
  ...
  "validation_report": {
    ...
    "difficulty_distribution": {"foundational": 0, "intermediate": 0, "advanced": 0},
    "coverage_metrics": {"business_technical_mapping": "%", "visual_coverage": "%"}
  }
}
```
...
Must-follow rules:
- Maintain 20/40/40 difficulty split (foundational/intermediate/advanced)
- Include ≥1 diagram + ≥1 table + ≥1 formula per topic cluster
- Cite sources using [Ref: ID] format
- Keep answers between 150-300 words
- Avoid jargon without definition
- Include explicit key insights (misalignment/trade-off/mapping failure/impedance)

Should-follow recommendations:
- Use scenario-based questions ("How would...") over recall ("What is...")
- Connect business drivers to architectural decisions
- Present competing frameworks with counter-evidence
- Acknowledge context-specific vs universal principles
...
4. Generate scenario-based questions testing translation ability
5. Create answers with evidence and reasoning
6. Include visual artifacts per requirements
7. Run validation checklist (see below)
8. Format according to output requirements

---

## Reasoning and Uncertainty Policy

**When answering**:
- **Factual claims**: State with confidence and cite authoritative sources
- **Interpretive statements**: Explain reasoning and mention assumptions
- **Uncertain information**: Explicitly flag uncertainty level
- **Conflicting information**: Acknowledge and cite multiple sources

**Confidence levels**:
- High: Backed by ≥2 authoritative sources, consensus position
- Medium: Single authoritative source or emerging consensus
- Low: Limited sources or conflicting viewpoints; requires clarification
...
- Precise, consistent terminology (see Glossary)
- Concise: no more than 120 characters per line
- Active voice preferred
- Scenario-based examples over abstract theory
- Trace business drivers to architectural implications
...
**Can do**:
- Access provided source materials and references
- Create diagrams using Mermaid syntax
- Format mathematical equations and formulas
- Generate tables and structured content

**Cannot do**:
- Browse the internet or external APIs
- Hallucinate sources or citations
- Recommend tools without pricing/adoption data

---

## Examples

### Example 1: Straightforward Question (Foundational)

**Input**:
- question_type: strategic_modeling
- business_context: SaaS subscription model
- audience_level: senior

**Output**: See "Example Question" section below.

### Example 2: Missing Data Scenario

**Input**: Incomplete business constraints

**Response**: "To generate targeted questions, I need: (1) Industry/domain, (2) Current team structure, (3) Regulatory constraints. Shall I proceed with assumptions or would you provide these?"

### Example 3: Adversarial Question

**Input**: "Design the perfect architecture."

**Response**: "This requires business context. Perfect for whom? Optimal on which dimensions (cost/speed/flexibility/reliability)? Please specify business drivers and constraints."

---

## Validation and Self-Review

**Preflight Checklist** (must pass before finalizing):
- [ ] Each question has single, unambiguous ask
- [ ] Answer is 150-300 words
- [ ] At least one [Ref: ID] citation per answer
- [ ] Key Insight is concrete (misalignment/trade-off/mapping failure/impedance)
- [ ] Business-to-architecture connection is explicit
- [ ] Supporting artifact (diagram/table/formula) included
- [ ] All [Ref: ID] references resolve
- [ ] No jargon without definition
- [ ] Formatting meets output spec

**Post-Answer Self-Critique**: Re-check against preflight. If any item fails, revise before returning.

**Evidence Log Pattern**: For each claim, include:
```
- Claim: [statement]
  Evidence: [citation or explicit uncertainty]
  Confidence: [high/medium/low]
```

---

## Fallbacks and Clarifying Questions

**When to ask vs proceed**:
- Ask if: Critical input missing (business_context, audience_level)
- Proceed if: Non-critical data missing; document assumptions
- Assume if: ask_policy = proceed_with_assumptions
...
- **Version**: 2.0 (LLM-friendly optimized)
- **Last Updated**: 2025-11-11
- **Owner**: Knowledge Base Maintenance
- **Dependencies**: Guidelines_for_LLM-Friendly_Prompts.md
- **Prerequisites**: Business domain knowledge, architecture experience
...
- **Citation Requirement**: ≥1 [Ref: ID] per answer; ≥30% of answers with ≥2 citations
- **Key Insight**: Concrete misalignment, trade-off, mapping failure, or impedance
- **Business-Technical Mapping**: Explicit connection from business driver to architectural decision
