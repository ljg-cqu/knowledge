...
☑ Format+quality specs ☑ Create/link decision ☑ Verified sources ☑ Alt text+captions+fallback ☑ Decision-critical only ☑ Text-media consistency+validation
...
**Test Description:** Check that validates format+quality specs, create/link decision, credible sources+link status (HTTP 200), WCAG 2.1 AA accessibility+fallback, decision-critical-only significance, and text-media consistency/integration for all multimedia.