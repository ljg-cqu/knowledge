...
- **Blocks Decision**: Case provides insights that prevent or enable critical architectural, business, or strategic choices
- **Creates Risk**: Failure mode or success pattern directly relates to material threats (outages, financial loss, security breaches)
- **Affects ≥2 Stakeholder Roles**: Multi-team lessons (e.g., DevOps + Security patterns, PM + Architect trade-offs)
- **Requires Action**: Lessons applicable to 1-6mo implementation decisions (not purely historical)
- **Quantified Impact**: Measurable case outcomes (downtime hours, cost $, users affected, performance %, revenue impact)
